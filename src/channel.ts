  ...
  base: {
    ...
    agentPrompt: {
      messageToolHints: ({ cfg, accountId }: { cfg: OpenClawConfig; accountId: string }) => {
        ...
        return [
          "### Apple Mail Channel Behavior",
          "- Email is an ASYNC channel. Each response you write becomes a separate email reply in the same thread.",
          "- To reply: simply write your response as normal text in your turn.",
          "- **DO NOT use the `message` tool or any send tool for email replies.**",
          "- The channel automatically delivers your assistant text responses to the email thread.",
          "",
          "### Thread Context",
          "- When the user emails you, the FULL email thread history is included in the message you receive.",
          "- Look for `=== EMAIL THREAD HISTORY ===` block at the top - it contains all prior messages in this thread.",
          "- Each prior message is labeled `[YOU - Assistant]` (your past replies) or `[User]` (their messages).",
          "- The current new message is at the bottom under `=== CURRENT MESSAGE ===`.",
          "- If there's no thread history block, this is a NEW conversation (first message in thread).",
          "- Use the thread history to maintain context across emails. Reference past messages when relevant.",
          "",
          "### For Follow-up Responses (after sub-agent completion)",
          "- When sub-agents complete, write your follow-up as plain text in your response.",
          "- The channel will auto-deliver the text as a new email reply in the same thread.",
          "- Do NOT try to send the email explicitly - just write what you want to say.",
          "",
          "### Key Rules",
          "- Write plain text responses. They auto-deliver as email replies.",
